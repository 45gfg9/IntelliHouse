...
DateTime::DateTime(
    ...
    : y(y < 2000 ? y : y - 2000), m(m), d(d), hh(hh), mm(mm), ss(ss) {}

DateTime::DateTime(const DateTime &copy)
    : y(copy.y), m(copy.m), d(copy.d), hh(copy.hh), mm(copy.mm), ss(copy.ss) {}

String DateTime::toString() const {
  static char buf[20];
  snprintf_P(
      buf, 20, PSTR("20%02d/%02d/%02d %02d:%02d:%02d"), y, m, d, hh, mm, ss);
  return String(buf);
}
